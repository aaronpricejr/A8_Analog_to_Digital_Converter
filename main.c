...
int main(void)
{
    ...
    while (1)
    {
        samples[sample_idx] = ADC_read_blocking();
        sample_idx++;
        ...
        if (sample_idx >= NUM_SAMPLES)
        {
            ...
            uint16_t adc_avg_raw = (uint16_t)(adc_sum / NUM_SAMPLES);

            /*
             * Faster method:
             * Process raw counts first, then convert only display values.
             */
            adc_min_mv = ADC_apply_cal(adc_min_raw);
            adc_avg_mv = ADC_apply_cal(adc_avg_raw);
            adc_max_mv = ADC_apply_cal(adc_max_raw);

            LPUART_print_ADC_table(adc_min_raw,
                                   adc_max_raw,
                                   adc_avg_raw,
                                   adc_min_mv,
                                   adc_max_mv,
